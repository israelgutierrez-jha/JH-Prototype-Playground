...
4. **Now collect the remaining details, one field at a time, in this order: designer name → prototype name → tags → description.** Don't dump all of these into one message — give each its own turn, and pick the presentation that fits the field:
   - **Designer name.** Check for an existing designer profile first: look for `.designer.local.json` at the repo root. If it exists and its `name` field is non-empty, kebab-case it (lowercase, non-alphanumeric → `-`, collapse/trim hyphens) and use that as the designer folder — skip asking. Otherwise, list the folders under `src/prototypes/*/` first; if any exist, ask with the `AskUserQuestion` tool: offer the existing designer folder names as options (recommend whichever seems like a returning designer) so they reuse their folder, plus the built-in "Other" for a new name. If there are no prototypes yet, or `AskUserQuestion` isn't available in the current tool (e.g. Cursor), just ask directly: "What's your name? (used as the folder name, e.g. `jack-henry`)" Either way, after asking, write `.designer.local.json` as `{"name": "<answer as given>", "onboarded": true}` (create or overwrite) so future runs and the browser app pick it up without asking again. This file is gitignored and local to this machine — never commit it.
