...
0. **Check for an existing designer profile first.** Look for `.designer.local.json` at the repo root. If it exists and its `name` field is non-empty, kebab-case it (lowercase, non-alphanumeric → `-`, collapse/trim hyphens) and use that as the designer folder — skip the "Designer name" question in step 1 entirely. Otherwise, ask as described in step 1, then write `.designer.local.json` as `{"name": "<answer as given>", "onboarded": true}` (create or overwrite it) so the browser app and future runs pick it up without asking again. This file is gitignored and local to this machine — never commit it.
