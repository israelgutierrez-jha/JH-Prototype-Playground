...
- A password gate (`proto-password-gate.ts`) in front of the gallery itself (shared password, set on the internal Settings page, hash lives in `src/config/external-access.ts`) and, independently, in front of any prototype whose own `meta.ts` has a `passwordHash` (set per-prototype via that prototype's own "Prototype settings" dialog).

**How a designer marks a prototype for the external gallery:** open the prototype → pencil icon → "Prototype settings" → toggle "Show in external (CU) gallery" and optionally set a password → Save (writes `public`/`passwordHash` to that prototype's own `meta.ts`, same as title/description already do) → commit and push to `main`.
